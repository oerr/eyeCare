...
namespace CSHTML5.Extensions.Plotly
{
    ...
    {
        public static bool JSLibraryWasLoaded; // Remembers whether the JS library was loaded in order not to load it twice:

        public ChartControl()
        {
            // Set default values:

            //new Windows.UI.Xaml.Media.Brush
        ...
        /// </summary>
        /// 
        //sTypedArrayJS : ms-appx:/ChartControl/typedarray.js
        //sPlotlyminJS  : ms-appx:/ChartControl/plotly.min.js
        public static async Task Initialize(string sTypedArrayJS, string sPlotlyminJS)
        {
            if (!JSLibraryWasLoaded)
            {
                // Load the "typedarray.js" polyfill for IE compatibility:
                await Interop.LoadJavaScriptFile(sTypedArrayJS);

                // Load the "plotly.js" library:
                await Interop.LoadJavaScriptFile(sPlotlyminJS);
